...
namespace ogame {
  namespace gui {
    ...
    inline
    void BuildingsView::onActionTriggered(const player::GeneralDataModel& model) {
      // Process this action.
      try {
        ...
        std::cerr << "[BUILDINGS] Caught exception while setting up buildings view:" << std::endl << e.what() << std::endl;
      }
    ...
    inline
    LabelledPictureShPtr BuildingsView::createLabelledPictureContainer(const std::string& name,
